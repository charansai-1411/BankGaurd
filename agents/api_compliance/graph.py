...
from agents.api_compliance.prompts import (
...
def initialize_state(state: AgentState):
    domain = state.get("agent_domain") or "api_compliance"
...
def tool_caller(state: AgentState):
    ...
    if decision == "DELEGATE":
        ...
        publish_agent_event(job_id, domain, "tool_caller", "delegating", f"Delegating chunk to peer '{target}' (call depth: {call_depth + 1})")
        delegated_findings = call_agent(target, query, chunk_text, call_depth, job_id=job_id)
        publish_agent_event(job_id, domain, "tool_caller", "done", f"Delegation to '{target}' completed with {len(delegated_findings)} findings.")
    ...
    # 2. Handle Investigation (Vector Search specs namespace)
    ...
    if retry == 0:
        ...
        try:
            ...
            query = f"{prev_query} REST API"
            
    # Search OpenAPI specifications table
    target_namespace = "api_specs"
    doc_id = chunk.get("metadata", {}).get("document_id")
    
    publish_agent_event(job_id, domain, "tool_caller", "vector_search", f"Searching OpenAPI specs: '{query}'")
...
def gap_analyzer(state: AgentState):
    ...
    if max_sim < 0.60:
        finding = {
            ...
            "has_gap": True,
            "coverage_level": "none",
            "gap_description": "No compliance evidence could be retrieved in OpenAPI specifications after retry cycles.",
            "cited_text": "",
    ...
    evidence_text = "\n\n".join([
        f"Result {i+1} (Path: {r['metadata'].get('path', 'unknown')}, Method: {r['metadata'].get('method', 'unknown')}, Sim: {r['similarity']:.2f}):\n{r['content']}"
        for i, r in enumerate(search_results)
    ]) if search_results else "No matching OpenAPI spec schemas found."
